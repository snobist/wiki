...
- The "popularity exception" for zero-death high-demand titles shipped as **Phase 2** of
  ...
  (`description`, `nationality`; `gender`/`alternateName` proposed). See [[wiki/seo-indexing]].
...
- `person_is_indexable` + `Person.indexable` wired into page + sitemap. Fixed ~12,500 soft-404s.
  Verified live. Open: `MIN_CREDITS` value; whether filmography-alone (no life data) qualifies. See [[wiki/seo-indexing]].
...
- See [[wiki/pipeline-science]] — actuarial survival modeling, Bayesian death fusion, record linkage,
