...
| OCI (stillcasting + MAS host) `130.61.219.74`, user `ubuntu` | `ssh -o ServerAliveInterval=15 -i <key> ubuntu@130.61.219.74` (from the Mac; Cowork VM cannot open SSH) | SSH key (1675 B, RSA PEM) | Backup: `~/Library/CloudStorage/OneDrive-OracleCorporation/private_folder/stillcasting/ssh-key-2026-03-18.key` (+ `INSTRUCTIONS.md` recovery notes, + `claude-export-2026-08-29/` with old wikis & transcript). File is OneDrive **cloud-only** — must be downloaded ("Always keep on this device") before use; `chmod 600`. Local working copy: `~/.ssh/oci-mas.key` (installed 2026-09-14 by `bin/stillcasting-diag.sh`) | SSH verified working from Claude Code 2026-09-14; see [[infra-deploys]] |
| stillcasting repo (private) `github.com/snobist/stillcasting.app` | `git@github.com:snobist/stillcasting.app.git`; local clone `~/Documents/Private_Projects/stillcasting` (branch `develop`) | SSH key `wiki-sync` (same `Host github.com` entry) | `~/.ssh/id_ed25519_wiki` | verified 2026-09-14; branches: main, develop, fix/import-cascade |
| financial-research-mas repo (private) `github.com/snobist/mas` | `git@github.com:snobist/mas.git`; local clone `~/Documents/Private_Projects/financial-research-mas` | SSH key `wiki-sync` (same `Host github.com` entry) | `~/.ssh/id_ed25519_wiki` | verified 2026-09-15; branches: main (live), develop, feature/*, design/* |
| stillcasting.app prod | Docker on OCI box, Caddy/nginx, Postgres | SSH | as above | see [[stillcasting-index]] |
| Google Search Console + GA4 (stillcasting) | Domain property `sc-domain:stillcasting.app` (Restricted user) and the GA4 property (Viewer) via service account `stillcasting-readonly@my-trial-project-198215.iam.gserviceaccount.com` (GCP project `my-trial-project-198215`; Search Console API + Analytics Data API enabled) | service-account JSON key, scopes `webmasters.readonly` + `analytics.readonly` | `.secrets/gsc-ga4-service-account.json` (chmod 600) | verified 2026-09-24 (GSC query OK from the Mac with `google-auth`+`requests` under python3.9 `--user`); GA4 property id: see [[gsc-performance]] |
