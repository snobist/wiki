...
| --- | --- | --- | --- | --- |
| Wiki git remote (private) | `bin/sync.sh` | HTTPS + token | `.secrets/git-token` (+ `.secrets/git-remote` for URL) | to set up — Alex to provide repo URL + PAT |
...
| OCI (stillcasting + MAS host) | SSH to the VPS | SSH key | `~/.ssh/` on the Mac (key name unknown — fill in) | unverified; see [[infra-deploys]] |
